...
def needs_summary(available_df, user_picked_names: List[str]) -> str:
...
def choose_strategy(
    ...
    preferred_name: Optional[str] = None,    # default you prefer (e.g., Hero RB + WR Flood)
    ...
    If `preferred_name` is provided, we give it a small boost; if it’s close to the top,
    we choose it.
    """
    if available_df is None or getattr(available_df, "empty", True):
